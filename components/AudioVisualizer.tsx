  ...
  useEffect(() => {
    ...
    const resize = () => {
      ...
      if (parent) {
        ...
        if (settings.performanceMode && rect.width > 1280) {
    ...
    const renderFrame = () => {
      ...
      const drawStars = (count: number, syncToBeat: boolean) => {
        ...
        starsRef.current.forEach(s => {
          ...
          if (s.isBeacon && finalOpacity > 0.3) {
      ...
      const drawDancer = (x: number, y: number, dancerScale: number, style: number, settings: VisualizerSettings) => {
        ...
        if (style === 1) { 
          bounce = (sLow / 255) * 45 * dancerScale;
          hipSway = Math.sin(time * 5) * 18 * dancerScale;
          headWobble = Math.sin(time * 10) * (sHigh / 255) * 15 * dancerScale;
          armWaveL = Math.sin(time * 11) * (sMid / 255) * 90 * dancerScale;
          armWaveR = Math.sin(time * 11 + Math.PI) * (sMid / 255) * 90 * dancerScale;
          stompL = Math.max(0, Math.sin(time * 8)) * (sLow / 255) * 35 * dancerScale;
          stompR = Math.max(0, Math.sin(time * 8 + Math.PI)) * (sLow / 255) * 35 * dancerScale;
        } else if (style === 2) {
      ...
      if (variant === 'stage-dancer') {
        ...
        const starCount = settings.performanceMode ? 80 : 150;
        const particleCount = settings.performanceMode ? 60 : 100;
        if (starsRef.current.length === 0) {
        ...
        if (journeyObjectsRef.current.length === 0) {
          ...
          journeyObjectsRef.current = Array.from({ length: settings.performanceMode ? 20 : 35 }, (_, i) => createObj(i));
        }
        ...
        const ringCount = settings.performanceMode ? 6 : 10;
        const centerX = width / 2;
        const centerY = height / 2;
        ...
        const bubbleCount = settings.performanceMode ? 25 : 50;
        if (particlesRef.current.length === 0) {
        ...
        const barCount = settings.performanceMode ? 80 : 120;
