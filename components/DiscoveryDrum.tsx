...
import { UsersIcon, XMarkIcon, HeartIcon } from './Icons';
...
const DEMO_USERS: UserProfile[] = [
    ...
    { id: 'd3', name: 'Sofia', avatar: 'https://i.pravatar.cc/150?u=13', age: 24, country: 'France', city: 'Paris', status: 'online', safetyLevel: 'green', bio: '', gender: 'female', blockedUsers: [], hasAgreedToRules: true, filters: { minAge: 18, maxAge: 99, countries: [], languages: [], genders: ['any'], soundEnabled: true } },
    ...
    { id: 'd6', name: 'Liam', avatar: 'https://i.pravatar.cc/150?u=16', age: 26, country: 'UK', city: 'London', status: 'online', safetyLevel: 'green', bio: '', gender: 'male', blockedUsers: [], hasAgreedToRules: true, filters: { minAge: 18, maxAge: 99, countries: [], languages: [], genders: ['any'], soundEnabled: true } },
      ...
      <div className="relative flex items-center justify-center w-full h-full">
        ...
        {instances.map((inst) => (
            <FlyingPhoto 
                ...
                maxRadius={maxRadius}
                onClick={() => { setSelectedUser(inst.user); setIsSpinning(false); }}
            />
        ))}
      </div>
      ...
      {selectedUser && (
        <div className="absolute inset-0 z-[200] flex items-center justify-center p-6 bg-black/90 backdrop-blur-3xl animate-in fade-in duration-500 pointer-events-auto">
          <div className="glass-panel w-full max-w-[320px] p-10 rounded-[4rem] border-blue-500/30 flex flex-col items-center text-center animate-in zoom-in duration-300 relative">
              ...
              <p className="text-[10px] text-slate-500 uppercase font-bold tracking-widest mb-10">{selectedUser.age} лет • {selectedUser.city}</p>
              <button 
                onClick={() => { onAgree(selectedUser); setSelectedUser(null); setIsSpinning(true); }}
